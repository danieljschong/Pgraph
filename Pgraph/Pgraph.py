...
class Pgraph():
    ...
    def plot_problem(self, figsize=(5, 10), padding=0, titlepos=0.95, rescale=2, box=True, node_size=3000):
        ...
        nx.draw(G, pos=pos, node_color='white', alpha=0.9, node_shape="o", edge_color='black', labels=node_labels,
                with_labels=True, node_size=node_size, bbox=label_options, width=weights, font_size=12)
        for aShape in nodeShapes:
            if aShape == "o":
                node_size1 = node_size
            else:
                node_size1 = node_size * 2
            nx.draw_networkx_nodes(G, pos=pos2, node_color='black', node_shape=aShape, nodelist=[sNode[0] for sNode in
                                                                                                 filter(lambda x: x[1][
                                                                                                                      "s"] == aShape,
                                                                                                        G.nodes(
                                                                                                            data=True))],
        ...
        for key, values in nx.get_node_attributes(G, 'type').items():
            if values == "raw_material":
                nx.draw_networkx_nodes(G, pos=pos, node_color='white', node_shape='v', nodelist=[key],
                                       node_size=node_size / 3 * 1.6)

            elif values == "product":
                nx.draw_networkx_nodes(G, pos=pos, node_color='white', node_shape='o', nodelist=[key],
                                       node_size=node_size / 3 * 2)
                nx.draw_networkx_nodes(G, pos=pos, node_color='black', node_shape='o', nodelist=[key],
                                       node_size=node_size / 3 * 1.25)
                nx.draw_networkx_nodes(G, pos=pos, node_color='white', node_shape='o', nodelist=[key],
                                       node_size=node_size / 3 * 0.75)
    ...
    def create_solver_input(self):
        ...
        self.problem_materials = {'ID': [], 'names': [], 'type': [], 'flow_rate_lower_bound': [],
                                  'flow_rate_upper_bound': [], 'price': [], 'time_unit': [], 'money_unit': [],
                                  'mass_unit': []}
        for n in G.nodes():
            if n[0] == "M":
        ...
        self.problem_operating = {'ID': [], 'names': [], 'fix_cost': [], 'proportional_cost': [], }
        for n in G.nodes():
            if n[0] == "O":
    ...
    def solve(self, system=None, skip_wine=False):
        ...
        if system == "Windows":  # support for windows
            rc = subprocess.run(
                [path + "pgraph_solver.exe", solver, path + "input.in", path + "test_out.out", str(max_sol)])
        elif system == "Linux":
            ...
            out_string = " ".join(
                ["wine", path + "pgraph_solver.exe", solver, path + "input.in", path + "test_out.out", str(max_sol)])
    ...
    def read_solutions(self):
        ...
        for i in range(len(lines) - 1, 1, -1):
            if lines[i][0] == " " or (
                    len(lines[i].strip()) > 0 and ":" not in lines[i] and "," not in lines[i] and "= " not in lines[
                i] and "End." not in lines[i]):  ## Attention for possible future changes
                if lines[i - 1][-3:] != "\n":
    ...
    def plot_solution(self, sol_num=0, figsize=(5, 10), padding=0, titlepos=0.95, rescale=2, box=True, node_size=3000):
        ...
        if self.solver in ["SSGLP", "INSIDEOUT", 2, 3] and len(goolist) != 0:
            ...
            pos = nx.rescale_layout_dict(pos, scale=rescale)
            pos2 = nx.rescale_layout_dict(pos2, scale=rescale)
            nx.draw(H, pos=pos, labels=labels1, node_color='white', alpha=0.9, node_shape='o',
                    edge_color=edge_color_list, with_labels=True, node_size=node_size, bbox=label_options,
                    width=weights, font_size=10)
            for aShape in nodeShapes:
                ...
                for node in node_list:
                    nx.draw_networkx_nodes(H, pos=pos2, node_color=H.nodes()[node]['color'], node_shape=aShape,
                                           nodelist=[node], node_size=node_size1)

            for key, values in nx.get_node_attributes(H, 'type').items():
                if values == "raw_material":
                    nx.draw_networkx_nodes(H, pos=pos, node_color='white', node_shape='v', nodelist=[key],
                                           node_size=node_size / 3 * 1.6)
                elif values == "product":
                    nx.draw_networkx_nodes(H, pos=pos, node_color='white', node_shape='o', nodelist=[key],
                                           node_size=node_size / 3 * 2)
                    nx.draw_networkx_nodes(H, pos=pos, node_color=H.nodes()[key]['color'], node_shape='o',
                                           nodelist=[key], node_size=node_size / 3 * 1.25)
                    nx.draw_networkx_nodes(H, pos=pos, node_color='white', node_shape='o', nodelist=[key],
                                           node_size=node_size / 3 * 0.75)
    ...
    def to_studio(self, path=None, file_name="studio_file.pgsx", verbose=False):
        ...
        for n in G.nodes():
            if n[0] == "M":
                ...
                if 'type' in G.nodes()[n]:
                    ...
                    attr = {"ID": str(list(G.nodes()).index(n) + 1), "Name": G.nodes()[n]["names"],
                            "Type": str(type_ind)}
                    ...
                    etree.SubElement(MPar_list[-1], 'Parameter',
                                     attrib={"Name": "price", "Prefix": "Price: ", "Value": str(price),
                                             "Visible": "false"})
                    etree.SubElement(MPar_list[-1], 'Parameter', attrib={"Name": "reqflow", "Prefix": "Required flow: ",
                                                                         "Value": str(flow_rate_lower_bound),
                                                                         "Visible": "false"})
                    etree.SubElement(MPar_list[-1], 'Parameter', attrib={"Name": "maxflow", "Prefix": "Maximum flow: ",
                                                                         "Value": str(flow_rate_upper_bound),
                                                                         "Visible": "false"})
                    etree.SubElement(MPar_list[-1], 'Parameter',
                                     attrib={"Name": "quantitytype", "Prefix": "Quantity type: ", "Value": "Mass",
                                             "Visible": "false"})
                    etree.SubElement(MPar_list[-1], 'Parameter',
                                     attrib={"Name": "measurementunit", "Prefix": "Measurement unit: ",
                                             "Value": "gram (g)", "Visible": "false"})
        ...
        for n in G.nodes():
            if n[0] == "O":
                ...
                for e in ee:
                    ...
                    attr = {"ID": str(global_edge_count), "BeginID": str(list(G.nodes()).index(e[0]) + 1),
                            "EndID": str(list(G.nodes()).index(e[1]) + 1), "Rate": str(ratio), "Title": str(ratio),
                            "ArrowOnCenter": "true", "ArrowPosition": "50"}
                ...
                for f in ff:
                    ...
                    attr = {"ID": str(global_edge_count), "BeginID": str(list(G.nodes()).index(f[0]) + 1),
                            "EndID": str(list(G.nodes()).index(f[1]) + 1), "Rate": str(ratio), "Title": str(ratio),
                            "ArrowOnCenter": "true", "ArrowPosition": "50"}
                    edge_list.append(etree.SubElement(Edges, 'Edge', attrib=attr))
                    nodes_list.append(etree.SubElement(edge_list[-1], 'Nodes'))  # Hanging
                    label_list.append(etree.SubElement(edge_list[-1], 'Label', attrib={"Text": str(ratio)}))
        ...
        for n in G.nodes():
            if n[0] == "O":
                ...
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "caplower", "Prefix": "Capacity, lower bound: ",
                                         "Value": str(capacity_lower_bound), "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "capupper", "Prefix": "Capacity, upper bound: ",
                                         "Value": str(capacity_upper_bound), "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "investcostfix", "Prefix": "Investment cost, fix: ",
                                         "Value": str(fix_cost), "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "investcostprop", "Prefix": "Investment cost, proportional: ",
                                         "Value": str(proportional_cost), "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "opercostfix", "Prefix": "Operating cost, fix: ", "Value": "-1",
                                         "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "opercostprop", "Prefix": "Operating cost, proportional: ",
                                         "Value": "-1", "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "workinghour", "Prefix": "Working hours per year: ", "Value": "-1",
                                         "Visible": "false"})
                etree.SubElement(OPar_list[-1], 'Parameter',
                                 attrib={"Name": "payoutperiod", "Prefix": "Payout Period: ", "Value": "-1",
                                         "Visible": "false"})
        ...
        if self.solver in ["SSGLP", "INSIDEOUT", 2, 3]:
            ...
            for i in range(len(goolist)):
                snum = i + 1
                attr = {"Index": str(i), "Title": "Solution #" + str(snum), "OptimalValue": str(goolist[i]),
                        "TotalTime": "0", "TotalMakespan": "0", "ObjectiveValue": str(goolist[i]),
                        "AlgorithmUsed": "INSIDEOUT"}
    ...
    def get_info(self):
        ...
        if self.solver in ["SSGLP", "INSIDEOUT", 2, 3]:
            OperatingUnit = [pd.DataFrame(x, columns=['Ratio', 'Names', 'Costs', 'Unit']).iloc[:, [1, 0, 2]] for x in
                             self.goplist]
            Materials = [pd.DataFrame(x, columns=['Names', 'Costs', 'MoneyUnit', 'Flow', 'FlowUnit']).iloc[:, [0, 3, 1]]
                         for x in self.gmatlist]
